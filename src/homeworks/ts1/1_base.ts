...
type ArrayType<T> = { value: T; number: number };

export const getNumberedArray = <T>(arr : Array<T>) : ArrayType<T>[] => 
  arr.map((value : T, number : number) : ArrayType<T> => ({ value, number }));
export const toStringArray = <T>(arr : ArrayType<T>[]) : string[] => 
  arr.map(({ value, number } : ArrayType<T>) : string => `${value}_${number}`);

type CustomerType = { id: number; name: string; age: number; isSubscribed: boolean };
//type TransformedCustomerType = Record<number, { name: string; age: number; isSubscribed: boolean }>;

const customers: CustomerType[] = [
  { id: 1, name: 'aaa', age: 1, isSubscribed: true },
  { id: 2, name: 'bbb', age: 2, isSubscribed: false },
];
